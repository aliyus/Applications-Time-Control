...
def evalMultiplexer(individual):
    ...
    result = sum(func(*in_) == out for in_, out in zip(inputs, outputs))/(2 ** MUX_TOTAL_LINES)
...
for ind in range(20):
    print('fitness:',pop[ind].fitness.values)
    print(len(pop[ind]))
    print(bool(pop[ind].fitness.values))
    print(str(pop[ind]))  

for ind in range(len(population)):
    print('fitness:',population[ind].fitness.values)
    print(len(population[ind]))
    print(bool(population[ind].fitness.values))
    print(str(population[ind]))    
    
    
    if not ind.fitness.valid:
        counteval =+ 1
        xo, yo, zo, = toolbox.evaluate(ind)
...
def main():
    ...
    #   ant.parse_matrix(trail_file)
      ...
      #       STEADY-STATE
      #==============================
    tag = f'Ph6_SSGP_prog-{PARITY_FANIN_M}L{Prob}'#Standard GP - Steady State
...
#        population = toolbox.population(n=50)
    ...
    for poolsize in poolsizelist:
     ...
     #==============================
    tag = f'Ph6_SSGP-BC_prog-{PARITY_FANIN_M}L{Prob}'#Standard GP - Steady State
